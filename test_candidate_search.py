...
class CandidateSearchTests(unittest.TestCase):
    def test_extracts_and_deduplicates_candidates(self):
        ...
        self.assertEqual(
            [item["name"] for item in candidate_search._extract_candidates(response, 5)],
            ["Alice Example", "Bob Example"],
        )

    def test_find_candidates_uses_key_file_and_returns_json_shape(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            key_file = Path(temp_dir) / ".exa"
            key_file.write_text("EXA_API_KEY=test-key\n", encoding="utf-8")
            with patch.object(candidate_search, "_request_exa", return_value={"results": []}) as request:
                result = candidate_search.find_candidates("Data Science", config_path=key_file)
        self.assertEqual(result, {"position_name": "Data Science", "candidates": []})
    ...
    def test_rejects_empty_position_name(self):
        with self.assertRaisesRegex(ValueError, "must not be empty"):
            candidate_search.find_candidates("  ")
